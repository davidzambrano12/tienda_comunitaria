  ...
  async crear(createPagoDto: CreatePagoDto, usuarioId: number) {
    ...
    if (!venta) {
      throw new NotFoundException(
        `La venta con ID ${createPagoDto.ventaId} no existe`,
      );
    }
    ...
    if (createPagoDto.monto > saldoPendiente + 0.01) {
      // Pequeño margen por decimales
      throw new BadRequestException(
        `El monto del pago ($${createPagoDto.monto}) excede el saldo pendiente ($${saldoPendiente.toFixed(2)})`,
      );
    }
    ...
    await this.auditoriaService.registrar(usuarioId, 'REGISTRO_PAGO', 'PAGOS', {
      pagoId: guardado.id,
      ventaId: guardado.ventaId,
    });
