...
export class ReportesService {
  private readonly MS_REPORTES_URL =
    process.env.MS_REPORTES_URL || 'http://localhost:8001';
  ...
  async generarReporteDiarioPDF(): Promise<Buffer> {
    ...
    if (ultima) {
      console.log(
        `Última venta en BD: ID=${ultima.id}, Fecha=${String(ultima.fecha)}`,
      );
    }
  ...
  private async formatearYEnviar(
    ventasDb: Venta[],
    titulo: string,
  ): Promise<Buffer> {
    ...
    console.log(
      'Enviando datos al microservicio:',
  ...
  async generarReporteVentas(dto: GenerarReporteDto): Promise<Buffer> {
    try {
      ...
      throw new HttpException(
        'Error al generar el reporte PDF en el microservicio.',
  ...
  async verificarSaludMicroservicio(): Promise<boolean> {
    try {
      await firstValueFrom(
        this.httpService.get(`${this.MS_REPORTES_URL}/health`, {
          timeout: 5000,
        }),
      );
      return true;
